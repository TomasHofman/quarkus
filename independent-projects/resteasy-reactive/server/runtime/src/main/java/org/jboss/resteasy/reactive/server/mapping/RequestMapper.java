...
public class RequestMapper<T> {
    ...
    public RequestMatch<T> map(String path) {
        ...
        for (PathMatcher.PathMatch<ArrayList<RequestPath<T>>> match : matches) {
            var result = mapFromPathMatcher(path, match);
            if (result != null) {
                return result;
            }
        }
    ...
    /**
     * Match the path to all possible UriTemplates. Returns all matches in priority order.
     *
     * @param path path to search UriTemplate for
     * @return list of all RequestMatches, may be empty but never null
     */
    public List<RequestMatch<T>> mapAll(String path) {
        List<RequestMatch<T>> allMatches = new ArrayList<>();

        List<PathMatcher.PathMatch<ArrayList<RequestPath<T>>>> matches = requestPaths.match(path);
        for (PathMatcher.PathMatch<ArrayList<RequestPath<T>>> match : matches) {
            collectAllMatches(path, match, allMatches);
        }

        // the following code is meant to handle cases like https://github.com/quarkusio/quarkus/issues/30667
        if (allMatches.isEmpty()) {
            collectAllMatches(path, requestPaths.defaultMatch(path), allMatches);
        }

        return allMatches;
    }

    private void collectAllMatches(String path, PathMatcher.PathMatch<ArrayList<RequestPath<T>>> pathMatch,
            List<RequestMatch<T>> collector) {
        var value = pathMatch.getValue();
        if (value == null) {
            return;
        }
        ...
        for (int index = 0; index < value.size(); index++) {
            ...
            int matchPos = pathMatch.getMatched().length();
            ...
            if (fullMatch || doPrefixMatch) {
                ...
                collector.add(new RequestMatch(potentialMatch.template, potentialMatch.value, params, remaining));
            }
        }
    }
